...
public class YoutubeVideo {
    ...
    public File download(Format format, File outDir) throws IOException, YoutubeException {
        ...
        String fileName = videoDetails.title().replace("CCTV少儿","") + "." + format.extension().value();
    ...
    public void downloadAsync(final Format format, File outDir, final OnYoutubeDownloadListener listener) throws IOException, YoutubeException {
        ...
        String fileName =  videoDetails.title() +"    "+videoDetails.description()+"    "+videoDetails.keywords()+ "." + format.extension().value();
...
//            fileName = videoDetails.title() + "(" + i++ + ")" + "." + format.extension().value();
              fileName =  videoDetails.title() +"    "+videoDetails.description()+"    "+videoDetails.keywords()+ "." + format.extension().value();
    ...
    public VideoFormat getGoodFormate(YoutubeVideo video){


        List<VideoFormat> videoWithQuality = video.findVideoWithQuality(VideoQuality.hd2880p);


        if (videoWithQuality!=null&&videoWithQuality.size()>0&& videoWithQuality.get(0).url()!=null ){
            return videoWithQuality.get(0);
        }


        videoWithQuality = video.findVideoWithQuality(VideoQuality.hd2160);


        if (videoWithQuality!=null&&videoWithQuality.size()>0&& videoWithQuality.get(0).url()!=null&& videoWithQuality.get(0).url()!=null){
            return videoWithQuality.get(0);
        }

        videoWithQuality = video.findVideoWithQuality(VideoQuality.hd1080);


        if (videoWithQuality!=null&&videoWithQuality.size()>0&& videoWithQuality.get(0).url()!=null){
            return videoWithQuality.get(0);
        }
        videoWithQuality = video.findVideoWithQuality(VideoQuality.highres);
        ...
        videoWithQuality = video.findVideoWithQuality(VideoQuality.hd720);


        if (videoWithQuality!=null&&videoWithQuality.size()>0){
            return videoWithQuality.get(0);
        }

        videoWithQuality = video.findVideoWithQuality(VideoQuality.medium);


        if (videoWithQuality!=null&&videoWithQuality.size()>0){
            return videoWithQuality.get(0);
        }
        return  video.videoFormats().get(0);
    }
